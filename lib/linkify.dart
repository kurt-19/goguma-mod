...
List<LinkifyElement> extractLinks(String text, [NetworkModel? network]) {
  ...
  return lnk.linkify(text,
      linkifiers: linkifiers,
      options: lnk.LinkifyOptions(
        humanize: false,
        defaultToHttps: true,
      ));
}
...
TextSpan linkify(
  ...
  try {
    buffer = context.read<BufferModel>();
  } on ProviderNotFoundException {
    // ignore
  }
  try {
  ...
  CallPageArguments? callArgs;
  for (var elem in elements) {
    if (elem is LinkableElement) {
      var uri = Uri.tryParse(elem.url);
      var channelTarget = uri?.queryParameters['channel'] ?? '';
      var role = network?.isIrcOperator == true ? 'irc_operator' : 'member';
      var channelMembers = const <String>[];
      if (channelTarget.isNotEmpty && network != null && client != null) {
        var channelBuffer = buffer != null &&
                client.isChannel(buffer.name) &&
                client.isupport.caseMapping.equals(buffer.name, channelTarget)
            ? buffer
            : bufferList?.get(channelTarget, network);
      ...
      callArgs = CallPageArguments.tryParse(
        elem.url,
        target: buffer?.name,
        nick: client?.nick,
        role: role,
  ...
  var children = elements.map((elem) {
    if (elem is LinkableElement) {
      return TextSpan(
        text: elem.text,
        style: linkStyle,
...
bool containsCallLink(String text) {
  return extractLinks(text)
      .whereType<LinkableElement>()
      .any((elem) => CallPageArguments.tryParse(elem.url) != null);
}

String _callLinkLabel(String text) {
  var lower = text.toLowerCase();
  if (lower.contains('video call')) {
    return lower.contains('end') ? 'video call end' : 'video call start';
  }
  if (lower.contains('voice call')) {
    return lower.contains('end') ? 'voice call end' : 'voice call start';
  }
  return lower.contains('end') ? 'call end' : 'call start';
}
    ...
    while (text != '') {
      ...
      if (coords != null) {
        ...
        if (Platform.isAndroid) {
          text = url;
          url =
              'geo:${coords[0]},${coords[1]}?q=${coords[0]},${coords[1]}(Position)';
        } else if (Platform.isIOS) {
          text = url;
          url =
              'https://maps.apple.com/?ll=${coords[0]},${coords[1]}&q=Position';
        }
