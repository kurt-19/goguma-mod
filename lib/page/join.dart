...
class _JoinPageState extends State<JoinPage> {
  ...
  void _search(String query) async {
    ...
    for (var network in networkList.networks) {
      ...
      if (network.bouncerNetwork != null &&
          network.bouncerNetwork!.state != BouncerNetworkState.connected) {
        continue;
      }
      var client = clientProvider.get(network);
      futures.add(
          _searchNetworkChannels(query, network, client).then(handleActions));
      futures
          .add(_searchNetworkUsers(query, network, client).then(handleActions));
    }
  ...
  Future<Iterable<_Action>> _searchNetworkChannels(
      String query, NetworkModel network, Client client) async {
  ...
  Future<Iterable<_Action>> _searchNetworkUsers(
      String query, NetworkModel network, Client client) async {
...
class _JoinItem extends StatelessWidget {
  ...
  const _JoinItem({required _Action action}) : _action = action;
  ...
  Widget build(BuildContext context) {
    ...
    if (action is _JoinChannelAction) {
      ...
      return ListTile(
        ...
        trailing: trailing,
        title: title,
        subtitle: action.listReply.topic == ''
            ? null
            : Text(
      ...
      return ListTile(
        ...
        title: title,
        subtitle:
            isStubRealname(action.whoReply.realname, action.whoReply.nickname)
                ? null
                : Text(
...
class _JoinChannelAction extends _Action {
  ...
  const _JoinChannelAction(this.listReply, NetworkModel network)
      : super(network);
...
class _CreateChannelAction extends _Action {
  ...
  const _CreateChannelAction(this.channel, NetworkModel network)
      : super(network);
