...
class _NetworkDetailsPageState extends State<NetworkDetailsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (network.bouncerNetwork != null &&
      ...
      statusTitle =
          bouncerNetworkStateDescription(network.bouncerNetwork!.state);
    ...
    if (network.state == NetworkState.offline) {
      var canReconnect = context.read<ClientProvider>().canUseNetwork(network);
      if (!canReconnect) {
        statusSubtitle = 'Additional server subscription required';
      }
    ...
    if (client.caps.enabled.contains('sasl') &&
        network.state == NetworkState.online) {
      if (network.account != null) {
        ...
        children.add(ListTile(
          ...
          subtitle: Text(
              'You are logged in with the account "${network.account}" on this network.'),
          trailing: trailing,
        ));
      } else {
        children.add(ListTile(
          ...
          subtitle:
              Text('You are not logged in with an account on this network.'),
          trailing: ElevatedButton(
            onPressed: () {
    ...
    var buffers = context
        .watch<BufferListModel>()
        .buffers
        .where((buffer) => buffer.network == network)
        .toList();

    var bufferList = SliverList(
    ...
    if (!buffers.isEmpty) {
      ...
      children.add(Container(
        margin: const EdgeInsets.all(15),
        child: Text('${buffers.length} conversation$s',
            ...
            actions: [
              if (network.bouncerNetwork != null)
                IconButton(
                  ...
                  onPressed: () {
                    Navigator.pushNamed(
                        context, EditBouncerNetworkPage.routeName,
                        arguments: network.bouncerNetwork!);
                  },
                ),
              if (network.bouncerNetwork != null)
                IconButton(
